...
async function fetchLokiLogs(
    ...
    const { start = config.timeRange, end = "now", limit = config.maxLogs, query = '{thing!=""}' } = options;
...
function calculateThingMetrics(logsByThing: Record<string, LogEntry[]>): ThingInfo[] {
    return Object.entries(logsByThing).map(([thingName, logs]) => {
        ...
        const lastSeen =
            logs.length > 0 ? logs[logs.length - 1]?.timestamp || new Date().toISOString() : new Date().toISOString();
    ...
    for (const thing of thingMetrics) {
        ...
        if (thing.health.errorRate > config.errorThreshold) {
            anomalies.push({
                ...
                severity:
                    thing.health.errorRate > config.errorThreshold * 3
                        ? "critical"
                        : thing.health.errorRate > config.errorThreshold * 2
                          ? "high"
                          : "medium",
        ...
        if (thing.health.avgResponseTime > config.responseTimeThreshold) {
            anomalies.push({
                ...
                type: "slow_response",
                severity:
                    thing.health.avgResponseTime > config.responseTimeThreshold * 3
                        ? "critical"
                        : thing.health.avgResponseTime > config.responseTimeThreshold * 2
                          ? "high"
                          : "medium",
...
function calculateAverageResponseTime(logs: LogEntry[]): number {
    const responseTimes = logs.map((log) => log.parsed?.responseTime).filter((rt): rt is number => rt !== undefined);
...
function generateRecommendations(anomalies: SystemMetrics["anomalies"], thingMetrics: ThingInfo[]): string[] {
    ...
    if (unhealthyThings.length > 0) {
        recommendations.push(
            `🩺 ${unhealthyThings.length} Things are unhealthy. Review their configurations and dependencies.`
        );
    }
...
function generateReport(metrics: SystemMetrics, things: ThingInfo[]): void {
    ...
    console.log(
        `Healthy: ${metrics.healthyThings} (${((metrics.healthyThings / metrics.totalThings) * 100).toFixed(1)}%)`
    );
    console.log(
        `Degraded: ${metrics.degradedThings} (${((metrics.degradedThings / metrics.totalThings) * 100).toFixed(1)}%)`
    );
    console.log(
        `Unhealthy: ${metrics.unhealthyThings} (${((metrics.unhealthyThings / metrics.totalThings) * 100).toFixed(1)}%)`
    );
    ...
    for (const thing of things) {
        const statusIcon = thing.health.status === "healthy" ? "✅" : thing.health.status === "degraded" ? "⚠️" : "❌";
