    ...
    private setupRoutes(): void {
        ...
        this.app.get("/api/things/:name", (req, res) => {
            try {
                ...
                logger.error(
                    `Failed to get status for ${req.params.name}:`,
                    error
                );
                res.status(500).json({ error: "Internal server error" });
            }
        ...
        this.app.get("/api/things/:name/logs", async (req, res) => {
            try {
                ...
                const lokiUrl = new URL(
                    `http://${process.env.LOKI_HOSTNAME}:${process.env.LOKI_PORT}/loki/api/v1/query_range`
                );
                lokiUrl.searchParams.append("query", query);

                if (start) {
                    lokiUrl.searchParams.append(
                        "start",
                ...
                logger.error(
                    `Failed to get logs for ${req.params.name}:`,
                    error
                );
                res.status(500).json({ error: "Internal server error" });
            }
